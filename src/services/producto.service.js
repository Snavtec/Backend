...
export class ProductoService {
    static async crearProducto(data) {
        try {
            ...
            if (error instanceof Prisma.Prisma.PrismaClientKnowRequestError) {
                // https://www.prisma.io/docs/reference/api-reference/error-refence

                // el producto ya existe
                // la data es insuficiente
                // la fk () del tipo producto no existe
                // si es que queremeos eliminar y ese registro tiene relacion con otra tabla no permitira la eliminacion
                return {
    ...
    static async devolverProducto(id) {
        ...
        // }
        const producto = await prisma.producto.fundUnique({
            where: { id: +id },
            // si usamos el include no podemos usar el select y viceversa
    ...
    static async listarProductos() {
        ...
        const productosIterados = productos.map((producto) => {
            return {
                ...
                // el operador Logical OR ( || ) es si el primer es falso, undefined o null pasara a validar el segundo, si ese tambien es de alguno de esos valores su contenido sera nulo, caso contrario tomara el primero o el segundo
